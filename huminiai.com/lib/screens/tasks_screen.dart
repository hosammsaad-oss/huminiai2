...
class TasksScreen extends ConsumerWidget {
 ...
 void _showAddTaskDialog(BuildContext context, WidgetRef ref) {
  ...
  String selectedHQI = 'التزام'; // التصنيف الجديد للرادار
  ...
  showDialog(
    ...
    builder: (context) => StatefulBuilder(
      builder: (context, setState) => Directionality(
        ...
        child: AlertDialog(
          title: Text("إضافة مهمة ذكية ✨", style: GoogleFonts.tajawal(fontWeight: FontWeight.bold)),
          content: SingleChildScrollView( // لضمان ظهور العناصر في الشاشات الصغيرة
            child: Column(
              ...
              children: [
                ...
                // اختيار المدة (يومية/أسبوعية...)
                _buildDropdownLabel("الجدول الزمني:"),
                ...
                const SizedBox(height: 15),
                // اختيار تصنيف الرادار (HQI)
                _buildDropdownLabel("تأثيرها على رادارك:"),
                DropdownButton<String>(
                  value: selectedHQI,
                  isExpanded: true,
                  items: const ['التزام', 'سرعة', 'تواصل', 'تطور', 'دقة']
                      .map((cat) => DropdownMenuItem(value: cat, child: Text(cat)))
                      .toList(),
                  onChanged: (val) => setState(() => selectedHQI = val!),
                ),
              ],
            ),
          ),
              ...
              onPressed: () {
                if (controller.text.isNotEmpty) {
                  ref.read(lifeProvider.notifier).addTask(
                    ...
                    hqiCategory: selectedHQI, // نرسل التصنيف الجديد
                  );
...
Widget _buildDropdownLabel(String text) => Align(
  ...
  child: Text(text, style: GoogleFonts.tajawal(fontSize: 12, color: Colors.grey)),
);
