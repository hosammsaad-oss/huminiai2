...
String _calculateFinancialHealth(double currentExpenses, double targetGoal) {
  if (targetGoal <= 0) return "يرجى تحديد هدف مالي أولاً";
  
  double remainingBudget = targetGoal - currentExpenses;
  
  // إذا تجاوز الميزانية أصلاً
  if (remainingBudget <= 0) {
    return "🚨 لقد تجاوزت الميزانية! هوميني ينصح بإيقاف الصرف فوراً.";
  }

  // حساب عدد الأيام المنقضية من الشهر (افتراضياً أو فعلياً)
  int dayOfMonth = DateTime.now().day;
  double dailyAverage = currentExpenses / dayOfMonth;
  
  // توقع متى سينتهي المال
  int daysUntilBankrupt = (remainingBudget / dailyAverage).floor();
  
  if (daysUntilBankrupt < 3) {
    return "⚠️ خطر! بناءً على صرفك، ميزانيتك ستنتهي خلال $daysUntilBankrupt أيام فقط!";
  } else if (daysUntilBankrupt < 10) {
    return "📉 انتبه يا حسام، ميزانيتك قد لا تكفي لنهاية الشهر (تكفي لـ $daysUntilBankrupt أيام).";
  } else {
    return "✅ وضعك ممتاز! استمر بهذا المعدل وستوفر ${remainingBudget.toStringAsFixed(0)} ريال بنهاية الشهر.";
  }
}
  Future<void> _updateFinancialGoal(double newTarget) async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "تم تحديث ميزانيتك الجديدة: $newTarget ريال 🎯",
  ...
  Future<void> _transferToVault(double amount) async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "تم إيداع $amount ريال في خزنتك بنجاح! 💰",
            style: GoogleFonts.tajawal(),
          ),
  ...
  Future<void> _checkBudgetExceeding() async {
    ...
    if (goalDoc.exists) {
      ...
      if (usagePercentage >= 0.8 && usagePercentage < 1.0) {
        _showUrgentAlert(
          "⚠️ تنبيه الميزانية",
          "حسام، لقد استهلكت أكثر من 80% من ميزانية هدفك! انتبه لمصاريفك القادمة.",
        );
      } else if (usagePercentage >= 1.0) {
        _showUrgentAlert(
          "🚨 تجاوزت الحد!",
          "لقد تخطيت ميزانية الهدف المحددة بالكامل. هوميني يقترح مراجعة فورية لخطة الصرف.",
        );
      }
    }
  }
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Expanded(
              child: Text(message, style: GoogleFonts.tajawal(fontSize: 13)),
            ),
          ],
        ),
  ...
  Future<void> _startSavingsChallenge() async {
    _showAIPlanDialog(
      "هوميني يصمم تحديك...",
      "يتم تحليل نمط صرفك لإنشاء تحدي توفير مخصص لك يا حسام.",
    );
    try {
      final groq = GroqService();
      String prompt =
          "المستخدم حسام يريد تحدي توفير للأسبوع القادم. اقترح 'مبلغاً واقعياً' للتوفير وهدفاً صغيراً. الرد يجب أن يكون مشجعاً وقصيراً جداً.";
  ...
  Future<void> _generateWeeklyReport() async {
    ...
    _showAIPlanDialog(
      "هوميني يجهز تقريرك...",
      "جارٍ جمع وتحليل بيانات الأسبوع الماضي...",
    );
  ...
  void _simulateIncomingSMS() {
    String fakeSMS =
        "مصرف الراجحي: شراء عبر مدى بقيمة 120.00 ريال لدى هرفي. الرصيد المتاح: 5300.50 ريال.";
  ...
  void _analyzeSMSWithAI(String smsText) async {
    try {
      final groq = GroqService();
      String prompt =
          "استخرج المبلغ (أرقام فقط)، والمتجر، والتصنيف من هذه الرسالة: $smsText. رد بصيغة: المبلغ | المتجر | التصنيف";
...
Widget _buildRankCard(int challengesSolved, double savedInVault) {
  // 1. تحديد بيانات الرتبة
  String rank = "مبتدئ";
  IconData rankIcon = Icons.eco;
  Color rankColor = Colors.brown;

  if (savedInVault > 1000 || challengesSolved > 5) {
    rank = "أسطورة الادخار";
    rankIcon = Icons.workspace_premium;
    rankColor = Colors.amber;
  } else if (savedInVault > 500 || challengesSolved > 2) {
    rank = "مخطط ذكي";
  ...
  return AnimatedContainer(
    duration: const Duration(seconds: 1), // مدة تغيير اللون
    ...
    decoration: BoxDecoration(
      ...
      boxShadow: [
        BoxShadow(color: rankColor.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 5))
      ],
    ),
    child: AnimatedSwitcher(
      duration: const Duration(milliseconds: 500), // مدة تبديل النص والأيقونة
      child: Row(
        key: ValueKey<String>(rank), // ضروري لكي يعرف المحرك متى يغير التأثير
        children: [
          // أيقونة نابضة
          TweenAnimationBuilder(
            ...
            child: Icon(rankIcon, color: Colors.white, size: 45),
          ),
  ...
  Future<void> _generateAIAdviceWithGoal(double expenses, double target) async {
    _showAIPlanDialog(
      "هوميني يحلل هدفك...",
      "جارٍ مقارنة مصاريفك مع هدف التوفير المخطط له...",
    );
    try {
      final groq = GroqService();
      String prompt =
          "المستخدم حسام لديه هدف $target وصرف $expenses. أعط نصيحة قصيرة جداً.";
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text(
              "تم",
              style: GoogleFonts.tajawal(color: const Color(0xFF6B4EFF)),
            ),
          ),
        ],
      ),
  ...
  // 1. تعريف المتغيرات وتصفيرها عند كل تحديث للبيانات
  List<Transaction> liveTransactions = [];
  double currentExpenses = 1.0;

  if (snapshot.hasData && snapshot.data != null) {
    // 2. تحويل المستندات إلى قائمة كائنات Transaction وحساب المجموع
    liveTransactions = snapshot.data!.docs.map((doc) {
      final data = doc.data() as Map<String, dynamic>;
      final double amt = (data['amount'] as num?)?.toDouble() ?? 0.0;
      
      // إضافة المبلغ للمجموع الكلي فوراً
      currentExpenses += amt;
        ...
        date: (data['date'] as Timestamp?)?.toDate() ?? DateTime.now(),
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // 1. عرض الرتبة (افترضنا أنك تجلب بيانات الخزنة والتحديات)
                 
                  // 2. عرض نصيحة تحليل الفئات
                 _buildAIInsightCardWithText(_getSmartInsight(liveTransactions, 7500.0)),
                   const SizedBox(height: 15),
                  _buildBalanceCard(7500.0 - currentExpenses, currentExpenses),
                  const SizedBox(height: 15),
                 // تأكد أن القيم التي ترسلها هي double وليست null
                  _buildPredictionCard(
                   currentExpenses ?? 1.0, 
                   7500.0 // أو القيمة التي تجلبها من الداتا بيز
                ),
                  ...
                  // نضعه مباشرة بعد الـ Balance Card
  ...
  stream: FirebaseFirestore.instance
      ...
      .doc(userId)
      .collection('vault')
      .doc('info') // تأكد من اسم المستند لديك (غالباً info أو data)
      .snapshots(),
  builder: (context, vaultSnapshot) {
    double savedAmount = 0.0;
    if (vaultSnapshot.hasData && vaultSnapshot.data!.exists) {
      savedAmount = (vaultSnapshot.data!['balance'] as num).toDouble();
    }

    // الآن نرسل المبلغ الحقيقي للدالة
    return _buildRankCard(3, savedAmount); 
  },
),
                      ...
                      children: [
                        _buildHeaderButton("تحديث الخطة", Icons.bolt, () async {
                          var goalDoc = await FirebaseFirestore.instance
                              ...
                              .get();
                          if (goalDoc.exists) {
                            double target = (goalDoc.data()!['target'] as num)
                                .toDouble();
...
Widget _buildCombinedAICard({required String insightText, required String predictionText}) {
  return Container(
    ...
    child: Column(
      children: [
        Row(
          children: [
            const Icon(Icons.psychology, color: Color(0xFF6B4EFF)),
            const SizedBox(width: 10),
            Text("مساعدك الذكي", style: GoogleFonts.tajawal(fontWeight: FontWeight.bold)),
          ],
        ),
        const Divider(height: 25),
        Text(insightText, style: GoogleFonts.tajawal(fontSize: 13, color: Colors.grey[800])),
        ...
        Container(
          ...
          child: Row(
            children: [
              const Icon(Icons.analytics_outlined, size: 18, color: Color(0xFF6B4EFF)),
              const SizedBox(width: 8),
              Expanded(child: Text(predictionText, style: GoogleFonts.tajawal(fontSize: 12, color: const Color(0xFF6B4EFF)))),
            ],
          ),
        ),
      ],
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    "${totalVault.toStringAsFixed(2)} ريال",
          ...
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: "المبلغ المراد توفيره",
            ...
            ElevatedButton(
              ...
              onPressed: () =>
                  _transferToVault(double.tryParse(_vaultController.text) ?? 0),
              child: Text(
                "نقل إلى الخزنة",
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  "وفر أكثر من 200 ريال لتحصل على وسام جديد!",
                  style: GoogleFonts.tajawal(
                    fontSize: 12,
          ...
          ElevatedButton(
            onPressed: _startSavingsChallenge,
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFF6B4EFF),
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(10),
  ...
  Widget _buildHeaderButton(String label, IconData icon, VoidCallback onTap) {
    return Padding(
      ...
      child: TextButton.icon(
        onPressed: onTap,
        icon: Icon(icon, color: const Color(0xFF6B4EFF), size: 18),
        label: Text(
          label,
          style: GoogleFonts.tajawal(
            color: const Color(0xFF6B4EFF),
  ...
  // حماية: إذا كان الهدف صفر أو أقل، نجعل النسبة صفر لتجنب Infinity
  double progress = (targetGoal > 0) ? (currentExpenses / targetGoal).clamp(0.0, 1.0) : 0.0;
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: Colors.white,
        ...
        children: [
          ...
          Text(
            progress >= 1.0
                ...
                : "متبقي ${(targetGoal - currentExpenses).toStringAsFixed(0)} ريال للالتزام بخطتك",
            style: GoogleFonts.tajawal(
              fontSize: 12,
              color: progress > 0.9 ? Colors.red : Colors.grey[600],
            ),
          ),
        ],
      ),
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () => _updateFinancialGoal(
              double.tryParse(goalController.text) ?? currentTarget,
            ),
      ...
      child: Row(
        children: [
          Expanded(
            child: PieChart(
              PieChartData(
                sections: _getChartSections(transList),
...
Widget _buildPredictionCard(double current, double target) {
  String advice = _calculateFinancialHealth(current, target);
  bool isDanger = advice.contains("🚨") || advice.contains("⚠️");

  return Container(
    width: double.infinity,
    padding: const EdgeInsets.all(16),
    decoration: BoxDecoration(
      // تغيير اللون بناءً على حالة الصرف
      color: isDanger ? Colors.orange.withOpacity(0.1) : Colors.green.withOpacity(0.1),
      borderRadius: BorderRadius.circular(15),
      border: Border.all(color: isDanger ? Colors.orange : Colors.green, width: 1),
    ),
    child: Row(
      children: [
        Icon(isDanger ? Icons.warning_amber_rounded : Icons.auto_graph, 
             color: isDanger ? Colors.orange : Colors.green, size: 30),
        ...
        Expanded(
          child: Text(
            ...
            style: GoogleFonts.tajawal(
              ...
              color: isDanger ? Colors.orange[900] : Colors.green[900],
            ),
          ),
        ),
      ],
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          subtitle: Text(
            "${t.category} - ${t.date.day}/${t.date.month}",
            style: GoogleFonts.tajawal(fontSize: 12),
          ),
          trailing: Text(
            "-${t.amount.toStringAsFixed(2)} ريال",
            style: GoogleFonts.poppins(
              color: Colors.red,
              fontWeight: FontWeight.bold,
            ),
      ...
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(15),
            border: Border.all(
              color: isSaving ? Colors.green[200]! : Colors.orange[200]!,
            ),
          ),
          child: Row(
            children: [
              Icon(
                isSaving ? Icons.insights : Icons.priority_high,
  ...
  void _checkBudgetStatusAndNotify(double currentSpent, double target) async {
    ...
    if (usageRatio >= 1.0) {
      ...
      body =
          "يا حسام، لقد تخطيت ميزانيتك بمقدار ${(currentSpent - target).toStringAsFixed(0)} ريال.";
    ...
    if (title.isNotEmpty) {
      ...
      _showSmartNotification(
        title,
        body,
        usageRatio >= 1.0 ? Colors.red : Colors.orange,
      );
...
String _calculateFinancialHealth(double currentExpenses, double targetGoal) {
  try {
    // 1. إذا لم يتم تحديد ميزانية
    if (targetGoal <= 0) return "حدد ميزانيتك الشهرية لبدء التنبؤ الذكي 🎯";
    
    double remainingBudget = targetGoal - currentExpenses;
    
    // 2. إذا تم تجاوز الميزانية بالفعل
    if (remainingBudget <= 0) {
      return "🚨 تجاوزت الميزانية! هوميني ينصح بإيقاف الصرف فوراً.";
    }
    ...
    // 4. الحماية من القسمة على صفر (إذا لم يصرف المستخدم شيئاً بعد)
    if (currentExpenses <= 0) {
      return "بداية ممتازة للشهر! لم تصرف شيئاً حتى الآن. استمر هكذا ✅";
    }

    // 5. حساب المتوسط اليومي
    double dailyAverage = currentExpenses / dayOfMonth;

    // 6. التحقق من المتوسط قبل القسمة عليه (الحل الجذري لخطأ Infinity)
    if (dailyAverage <= 0) {
      return "وضعك المالي مستقر حالياً ✅";
    }

    // 7. حساب الأيام المتبقية
    double daysResult = remainingBudget / dailyAverage;
    
    // تأكد أن النتيجة ليست Infinity أو NaN قبل استخدام floor
    if (daysResult.isInfinite || daysResult.isNaN) {
       return "✅ ميزانيتك ممتازة وتكفيك لفترة طويلة جداً!";
    }

    int daysUntilBankrupt = daysResult.floor();
    
    if (daysUntilBankrupt < 3) {
      return "⚠️ خطر! ميزانيتك ستنتهي خلال $daysUntilBankrupt أيام فقط!";
    } else if (daysUntilBankrupt < 10) {
      return "📉 انتبه، ميزانيتك قد لا تكفي لنهاية الشهر (تكفي لـ $daysUntilBankrupt أيام).";
    } else {
      return "✅ وضعك ممتاز! ستوفر ${remainingBudget.toStringAsFixed(0)} ريال بنهاية الشهر.";
    }
  } catch (e) {
    // في حال حدوث أي خطأ غير متوقع، لا تغلق التطبيق
    return "هوميني يحلل بياناتك الآن... 🧠";
  }
}

String _getSmartInsight(List<Transaction> transactions, double target) {
  if (transactions.isEmpty) return "ابدأ بتسجيل عملياتك لنحلل نمط صرفك الذكي 🧠";

  // 1. حساب إجمالي الصرف لكل فئة
  Map<String, double> categoryTotals = {};
  for (var t in transactions) {
    categoryTotals[t.category] = (categoryTotals[t.category] ?? 0) + t.amount;
  }

  // 2. تحديد الفئة الأكثر صرفاً
  var topCategory = categoryTotals.entries.reduce((a, b) => a.value > b.value ? a : b);
  double percent = (topCategory.value / target) * 100;

  // 3. بناء النصيحة بناءً على النسبة
  if (percent > 40) {
    return "يا حسام، فئة **${topCategory.key}** تستهلك **${percent.toStringAsFixed(0)}%** من ميزانيتك. حاول تقليلها لتوفر مبالغ أكبر في الخزنة 💰.";
  } else if (categoryTotals.length > 3) {
    return "توزيع صرفك متوازن حالياً. استمر في مراقبة فئة **${topCategory.key}** فهي الأعلى هذا الأسبوع.";
  } else {
    return "أداء مالي مستقر! هل فكرت في نقل جزء من ميزانية **${topCategory.key}** إلى الخزنة الآن؟";
  }
}

Widget _buildAIInsightCardWithText(String insightText) {
  return Container(
    width: double.infinity,
    padding: const EdgeInsets.all(16),
    decoration: BoxDecoration(
